...
# ۳) شبکه مشترک gateway/Martin موجود باشد
docker network inspect map_services_net >/dev/null 2>&1 || docker network create map_services_net

# ۴) فایل OSM ایران موجود باشد
...
docker run --rm --env-file .env --add-host host.docker.internal:host-gateway \
  ...
  "import psycopg2,os; psycopg2.connect(host=os.environ['DB_HOST'],port=os.environ['DB_PORT'],\
dbname=os.environ['DB_NAME'],user=os.environ['DB_USER'],password=os.environ['DB_PASS']); print('DB OK')"
```
